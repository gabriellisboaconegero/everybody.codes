...
indexedMatrixToGraph neighbours (mi, mj, indexedMatrix) = Map.fromList . map (\(index, val) -> ((index, val), map (\(i, j) -> indexedMatrix !! (i*mj+j)) . neighbours mi mj $ index)) $ indexedMatrix
...
part1 input = recoverPath end dij
  where
    dij = dijkstra cost [start] . indexedMatrixToGraph neighbours $ ii'

    cost (_, v1) (_, v2) = distModulus (val v1) (val v2) 10 + 1
    ...
    start = head . filter ((== 'S') . snd) $ mat
    end   = head . filter ((== 'E') . snd) $ mat

part3 input = Map.lookup end dij
  where
    dij = dijkstra cost starts . indexedMatrixToGraph neighbours $ ii'
